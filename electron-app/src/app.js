// GanttGen Desktop App - Frontend Logic (Electron Edition)
...
async function loadPalettes() {
    try {
        const palettes = await api.getPaletteInfo();
...
async function handleFileSelection(filePath) {
    try {
        // Validate the file
        await api.validateInputFile(filePath);
        ...
        if (!state.outputDir) {
            try {
                const dir = await api.getDirname(filePath);
                state.outputDir = dir;
...
async function selectOutputDirectory() {
    try {
        ...
        if (dirPath) {
            ...
            elements.outputDir.value = dirPath;
        }
    } catch (error) {
...
async function openOutputFolder() {
    if (state.outputDir) {
        try {
            await api.shellOpen(state.outputDir);
        } catch (error) {
            console.error('Failed to open folder:', error);
        }
    } else if (state.lastResult?.html_path) {
        try {
            const dir = await api.getDirname(state.lastResult.html_path);
            await api.shellOpen(dir);
        } catch (error) {
            console.error('Failed to open folder:', error);
        }
    }
...
async function viewHtmlFile() {
    if (state.lastResult?.html_path) {
        try {
            await api.shellOpen(state.lastResult.html_path);
        } catch (error) {
